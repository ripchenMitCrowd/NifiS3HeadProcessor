 ...
 */
package org.example.processors.s3head;
...
import org.apache.nifi.processors.aws.s3.*;
...
@UseCase(
        description = "Head a specific file from S3",
        configuration = """
        ...
        The "Object Key" property denotes the fully qualified filename of the file to head. Typically, the FlowFile's `filename` attribute is used, so this property is \
        set to `${filename}`.
...
@MultiProcessorUseCase(
        ...
        keywords = {"s3", "state", "retrieve", "head", "all", "stream"},
        configurations = {
                ...
                The 'success' Relationship of this Processor is then connected to HeadS3Object.
                """
                ),
                @ProcessorConfiguration(
                        processorClass = S3HeadObject.class,
                        configuration = """
                "Bucket" = "${s3.bucket}"
                "Object Key" = "${filename}"
...
@MultiProcessorUseCase(
        ...
        keywords = {"s3", "state", "retrieve", "filter", "select", "head", "criteria"},
        configurations = {
                @ProcessorConfiguration(
                        processorClass = ListS3.class,
                        configuration = """
                ...
                For example, to head only files that are at least 1 MB and have a filename ending in `.zip` we would set the following properties:
                - "Routing Strategy" = "Route to 'matched' if all match"
                - "At least 1 MB" = "${s3.length:ge(1000000)}"
                - "Ends in .zip" = "${filename:endsWith('.zip')}"

                Auto-terminate the `unmatched` Relationship.
                Connect the `matched` Relationship to the S3HeadObject processor.
                """
                ),
                @ProcessorConfiguration(
                        processorClass = S3HeadObject.class,
...
@MultiProcessorUseCase(
        ...
        configurations = {
                ...
                The 'success' relationship is connected to S3HeadObject.
                """
                ),
                @ProcessorConfiguration(
                        processorClass =  S3HeadObject.class,
...
public class S3HeadObject extends AbstractS3Processor {
        ...
        if (encryptionService != null) {
            ...
            if (strategyName.equals(AmazonS3EncryptionService.STRATEGY_NAME_SSE_S3) || strategyName.equals(AmazonS3EncryptionService.STRATEGY_NAME_SSE_KMS)) {
                problems.add(new ValidationResult.Builder()
                        ...
                        .valid(false)
                        .explanation(encryptionService.getStrategyDisplayName() + " is not a valid encryption strategy to head objects. Decryption will be handled automatically " +
                                "during the head of S3 objects encrypted with " + encryptionService.getStrategyDisplayName())
                        .build()
                );
            }
        }
    ...
    public List<ConfigVerificationResult> verify(ProcessContext context, ComponentLog verificationLogger, Map<String, String> attributes) {
        ...
        try {
            ...
            getLogger().error(String.format("Failed to head [%s] from Bucket [%s]", key, bucket), e);
            results.add(new ConfigVerificationResult.Builder()
                    .verificationStepName("HEAD S3 Object")
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) {
        ...
        final GetObjectMetadataRequest request = createGetObjectRequest(context, flowFile.getAttributes());

        final ObjectMetadata metadata = client.getObjectMetadata(request);
        if (metadata == null) {
            IOException ioe = new IOException("AWS refused to execute this request.");
    ...
    private GetObjectMetadataRequest createGetObjectRequest(final ProcessContext context, final Map<String, String> attributes) {
        final String bucket = context.getProperty(BUCKET_WITH_DEFAULT_VALUE).evaluateAttributeExpressions(attributes).getValue();
        final String key = context.getProperty(KEY).evaluateAttributeExpressions(attributes).getValue();
        final String versionId = context.getProperty(VERSION_ID).evaluateAttributeExpressions(attributes).getValue();
        final boolean requesterPays = context.getProperty(REQUESTER_PAYS).asBoolean();
